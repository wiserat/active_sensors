...
def wrong_notification(place_name, my_message, context, list_of_problems):
    ...
    my_body = """
                 Dobrý den, 
                 s lítostí Vám musíme oznámit, že Váš senzor přestal fungovat.
                 Konkrétně se jedná o senzor.
    """
    my_message.set_content(my_body)
    
    # radek kde place_name = radek
    # current_user.id = radek akorat sloupec kde current_user.id
